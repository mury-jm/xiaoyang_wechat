let wxCharts = require('../../js/wxcharts-min.js');
...
Page({
  ...
  generateMonth: function () {
    ...
    for (let i = 1; i <= month; i++) {
      this.data.monthData.unshift({ title: i >= 10 ? i : '0' + i, isSelected: false });
    }
  ...
  generateDay: function () {
    ...
    for (let i = 1; i <= day; i++) {
      this.data.dayData.unshift({ title: i >= 10 ? i : '0' + i, isSelected: false });
    }
  ...
  selectDay: function (e) {
    ...
    console.log('ceshihaoma',this.data.dayData)
    this.setData({
  ...
  selectData() {
    ...
    console.log(this.data.currentSelectMonth,'-------==3=====++++++++++++++++============-----------')
    //是否选择月份
    if (month == '') {
      //按年查询记账数据
      o.start = year + '-01-01';
    ...
    console.log(this.data.tabData,'slkdjf超级加倍')
    this.setData({
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        ty.forEach((v, i) => {
          ...
          res.result.data.forEach(item => {
            if (v == item.const_type) {
              tyDatas[v].unshift(item);
      ...
      fail: err => {
        wx.hideLoading();
        console.log('err [get_list_date] 云函数调用失败');
      }
    })
  ...
  formatPieData: function (costType) {
    ...
    this.data.recordDatas[costType].forEach(v => {
      ...
      for (let i = 0; i < types.length; i++) {
        if (types[i].type == v.img_type) {
          
          return;
        }
      }
      types.unshift({ type: v.img_type, name: v.img_title, cost: v.const, costType: v.const_type,  icon: v.icon_url});
    ...
    types.forEach((v, i) => {
      ...
      allCostTypeData.unshift(costTypeData);
      // console.log(allCostTypeData,'============================>demo')
      typeData.total = typeData.total.toFixed(2);

      typeData.ids = typeData.ids.join('@');

      this.data.typeDatas.unshift(typeData);
  ...
  selectCurrentTypeData: function (e) {
    ...
    console.log(e)
    console.log(o)
    console.log(this.data.recordDatas)
